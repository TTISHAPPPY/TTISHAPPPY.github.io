...
function tonav() {
  ...
  document.getElementById("page-name").innerText =
    document.title.split("TTchemistry")[0];
}
...
$.ajax({
  ...
  data: {
    key: "MOABZ-4ALWA-CMAK2-C3QJT-LSWYQ-RYFZO", // 这里要写你的KEY!!!
    output: "jsonp",
  },
  dataType: "jsonp",
...
function createtime1() {
  var grt = new Date("09/01/2023 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Oldpoem %c 你正在访问Oldpoemの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Fomalhaut🥝")
...
function createtime() {
  ...
  var start = new Date("09/01/2023 00:00:00"); // 旅行者1号开始计算的时间
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ? `<img class='boardsign' src='https://ttishappy.oss-cn-hangzhou.aliyuncs.com/img/TTchemistry/money.svg' title='什么时候能够实现财富自由呀~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 赛尔号 当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`
      : `<img class='boardsign' src='https://ttishappy.oss-cn-hangzhou.aliyuncs.com/img/TTchemistry/game.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 赛尔号 当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
...
if (localStorage.getItem("snow") == undefined) {
  localStorage.setItem("snow", "blcok");
}
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://ttishappy.oss-cn-hangzhou.aliyuncs.com/img/TTchemistry/8.webp);
    --darkmode-bg:url(https://ttishappy.oss-cn-hangzhou.aliyuncs.com/img/TTchemistry/6.webp);
    --mobileday-bg: url(https://ttishappy.oss-cn-hangzhou.aliyuncs.com/img/TTchemistry/2.webp);
    --mobilenight-bg: url(https://ttishappy.oss-cn-hangzhou.aliyuncs.com/img/TTchemistry/9.webp);
  }`;
}
// 切换背景主函数
function changeBg(s) {
